...
attachObject obj = case obj of
  BpfObjectLoaded h -> Control.do
    ...
    either
      ( Unsafe.toLinear
          (\pair -> case pair of (h', e) -> Control.pure (Left (e, BpfObjectLoaded h')))
      )
      ( Unsafe.toLinear
          ( \pair -> case pair of (h', link) -> Control.pure (Right (BpfObjectAttached h' link))
          )
      )
      r
...
splitLoaded
  :: (Dupable objRes)
  => BpfObject objRes linkRes 'Loaded
  %1 -> (objRes, BpfObject objRes linkRes 'Loaded)
...
splitAttached
  ...
  => BpfObject objRes linkRes 'Attached
  %1 -> (objRes, linkRes, BpfObject objRes linkRes 'Attached)
...
class HasObjRes objRes linkRes (s :: LifecycleState) where
  borrowObjRes
    :: BpfObject objRes linkRes s %1 -> (objRes, BpfObject objRes linkRes s)
        ...
        ( \restore -> do
            ...
            case attachResult of
              ...
              Right attachedObj -> do
                ...
                Ur result <-
                  restore (callback calleeObj)
                    `Exception.onException` runLinearUnit
                      ( Control.do
